...
@Suite struct ModifierTransitionsTests {
    ...
    private func modBinding(
        _ name: String, mods: Modifiers,
        apps: [String]? = nil,
        condition: Condition? = nil,
        onUp: Action? = nil
    ) -> Binding {
        Binding(
            name: name, trigger: .modifiersOnly, modifiers: mods,
    ...
    @Test func nonModifierOnlyBindingsIgnored() {
        let edges = transitions(
            [
                ...
                Binding(
                    name: "key", trigger: .key(0x69), modifiers: [.cmd],
    ...
    @Test func conditionGateFiltersTransitions() {
        ...
        #expect(
            transitions(
                b, prev: [], curr: [.lcmd],
                state: StateSnapshot(variables: ["on": 1])
            ).first?.edge == .entered)
    }
}
