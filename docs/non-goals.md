...
- **AX 1 つで動く軽さ（コア）**: コア機能は Accessibility 権限だけで動く。
  DriverKit / 仮想 HID デバイス / root daemon は要求しない。**唯一の例外は
  opt-in の v-key**: canon firmware が `&vkey <id>` で送る vendor usage page
  `0xFF31` / report ID `0x20` の 1-byte selector を読むため IOHIDManager +
  Input Monitoring（`kTCCServiceListenEvent`）を使う。config に v-key binding
  を書いたユーザだけが起動する経路で（`Controller.maybeStartVKeySource` が
  `configDeclaresVKeys()` で gate）、非 v-key ユーザは IOHIDManager を開かず
  Input Monitoring も求められない。**一般 HID interception / per-device
  routing / DriverKit は依然 non-goal**。
...
これらに反するものは **どんなに人気があっても non-goal**。唯一の例外は
v-key（上記）— 単一 vendor page を 1 デバイスから読むだけの、明示 opt-in かつ
最小権限の経路として個別に正当化済み。一般化（per-device routing / 一般 HID
interception / DriverKit）には拡張しない。
...
**通常キー**を物理デバイス単位で分岐させる（内蔵キーボードで抑止し canon
だけで発火）のは **依然 non-goal**。理由は技術的不能ではなく設計選択:

- CGEventTap には `kCGKeyboardEventKeyboardType` (HID country code) と
  eventSource ID 程度しか乗らず、**通常イベント**の物理デバイス identity は
  剥がれている
- これを得るには **IOHIDManager で通常キーボードの全 HID report を購読** +
  Karabiner 風の HID/CGEvent 突合、もしくは DriverKit 仮想 HID 経由が必要
- どちらも tap 一段の consume/pass 契約と **コアの「AX 1 つで動く軽さ」を
  喪失** する

**v-key との違い（重要）**: chord は v-key 用に IOHIDManager で dongle を
VID/PID（`0x1D50`/`0x615E`）マッチするが、読むのは canon が自分で定義した
vendor usage page（`0xFF31` / report ID `0x20`）の 1-byte selector **だけ**で、
通常キーボードの report は一切読まない。よって per-device な通常キー routing
にはならず、HID/CGEvent 突合も不要。v-key は「1 デバイスの 1 自前ページを
読む」狭い窓であって、ここで言う per-device 分岐ではない（§USP の例外）。
...
filter → chord で routing) のが正しい役割分担。（v-key の vendor-HID 読み取りは
これとは別物 — 通常キーの per-device filter は引き続き Karabiner の役割。）
...
- **Adapter 層の肥大化**: ChordAdapterMacOS は CGEventTap / AX / NSWorkspace /
  IOHIDManager（v-key の vendor-HID read）に限定された "OS と接する層" として
  定義されている (`Package.swift` コメント参照)。これらは全て *入力 / 権限*
  surface であり、*描画* surface（NSWindow / NSAttributedString）が入ると
  Adapter の責務境界が拡散する。
...
| Non-goal | Yes になる条件 |
|---|---|
| 1. mod-morph | chord が**通常キーの** remap を HID 層 (DriverKit 仮想 HID) で行う決定が別途されたとき（v-key の read-only vendor-HID 経路はこれに該当しない） |
| 2. per-device | 上記（通常キーの per-device 分岐が必要）+「内蔵キーボードでの誤発火」が複数ユーザから具体報告されたとき。注: v-key の IOHIDManager VID/PID マッチは自前 vendor page を読むだけで、この per-device routing には当たらない |
