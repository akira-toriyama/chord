...
- [x] dead-code 削除: Log.emit `mirrorToStderrOverride` 引数 (全 caller nil) / resolveBareAlias `body` 引数 (guard で即シャドウ) / cmdToggle 到達不能 branch (現 writer は ISO8601\t 前置 + "paused bindings=N" のみ → `\tpaused ` のみ生存) → PR #129
- [ ] (任意・低優先) 低リスク dedup: sockaddr_un setup (QueryServer/Control)、JSONEncoder 構築 (Schema/QuerySchema)、action-interception switch (handle/fireBindingAction)。behavior-equivalence 確認が要るため後回し。
